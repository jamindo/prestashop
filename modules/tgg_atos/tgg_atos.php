...
class tgg_atos extends PaymentModule {
	...
	public function getContent(){
		...
		$this->_autoCheck();
		if ($this->warning) $errors[] = $this->warning;
	 ...
	 */
	public function processResponse($Response, &$Customer, &$Order, &$Currency, &$amount, $mode = self::RESPONSE_MODE_POST) {
		global $cookie, $cart;
	...
	protected function _getMinAmount($payment_currency, $payment_n = FALSE) {
		...
		$default_currency = $this->_getFallBackCurrency();
		if ($default_currency['id_currency'] == $payment_currency['id_currency']) return $this->_round($minAmount, 0);
	...
	public function canProcess($cart, $payment_n = FALSE) {
		if ($payment_n && !$this->_get('BOOL_'.$payment_n.'TPAYMENT')) return FALSE;
		if (!$this->_hasTransacIDAvailable()) return $this->l('Payment by card is unavailable until tomorrow, we apologize for the inconvenience.');
		$payment_currency = $this->_getCurrencyToUse($cart);
		if ($min_amount = $this->_getMinAmount($payment_currency, $payment_n)) {
			...
			if ($min_amount > $amount) return sprintf($this->l('This payment method requieres a minimum cart amount of %s to be used, we apologize for the inconvenience.'), Tools::displayPrice($min_amount, $payment_currency));
		}
		return TRUE;
	}
	...
	public function getFees($amount, &$payment_currency, $payment_n = 1, $type = self::FEES_TOTAL) {
		...
		switch ($type) {
			...
			case self::FEES_FIXED:
				if (empty($payment_currency)) $payment_currency = $this->_getCurrencyToUse($cart);
				$fees = $this->_getFloat($prefix.'PAYMENT_FEES');
				$default_currency = $this->_getFallBackCurrency();
				if ($default_currency['conversion_rate'] == $payment_currency['conversion_rate']) {
					return $fees;
				} else {
					return $this->_round($fees / $default_currency['conversion_rate'] * $payment_currency['conversion_rate'], min(_PS_PRICE_DISPLAY_PRECISION_, $payment_currency['nb_decimals']));
				}
			break;
	...
	protected function _generateTransactionID() {
		...
		if (!$tid || $error) {
			$this->_debug_error(
				$this->l('Unable to generate Transaction ID'), 
				'error',
				array('{error}' => sprintf(
					$this->l('Transaction ID generation resulted with a failure, check the existance of the DB table %s. A misconfiguration of DB user privileges is often the cause of the failure to create this table while installing. Uninstall module, check DB user privileges (CREATE TABLE statement on the DB is needed) and the reinstall.%s%s'),
					_DB_PREFIX_.$this->name.'_transactions_today',
					"\n\n",
					$error
				))
			);
	...
	protected function _debug_error($title, $tplname, $vars) {
		...
		if ($this->_ipmask_proxyaware_check() && ($text = file_get_contents(sprintf('%s/mails/fr/%s.txt', rtrim(dirname(__FILE__),'/\\'), $tplname)))) {
			...
			$msg = sprintf('<h2>Tgg_Atos: %s</h2><pre>%s</pre></div>',
				$title,
				$text
			);
	...
	protected function _logInstall(&$messages) {
		try {
			...
			$this->_debug_error(
				$this->l('Installation malfunction'), 
				'error', 
				array(
					'{error}' => sprintf(
	...
	protected function _willSwitchCurrency($cart) {
		$payment_currency = $this->_getCurrencyToUse($cart);
		if ($cart->id_currency == $payment_currency['id_currency']) return FALSE;
		return $payment_currency;
	}
		...
		foreach ($showtoip as $mask) {
			...
			switch (count($mask)) {
				case 1:
					...
					break;
				case 2:
					if (empty($_SERVER['HTTP_X_FORWARDED_FOR'])) continue;
