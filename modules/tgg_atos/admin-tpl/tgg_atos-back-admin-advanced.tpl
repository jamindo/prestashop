...
<div class="margin-form">
	...
	<a href="#" onclick="var f = $('#field_errors_showtoip'); f.val(f.val() ? f.val() + '; ' + $(this).text() : $(this).text()); return false;" title="{l s='Click to add' mod='tgg_atos'}">{$browsing_through}</a>
	<br />
...
<div class="margin-form">
	<br />
	<label class="t clearfix" style="width: 500px; line-height: 1.4em; text-align: left;"><input type="checkbox" name="bool_binaries_in_path" style="vertical-align: text-bottom;" onclick="{literal}if ($(this).attr('checked')) {$('#field_bin_path').attr('disabled', 'disabled');} else {$('#field_bin_path').removeAttr('disabled');}{/literal}" value="1" {if $bool_binaries_in_path} checked="checked"{/if}/>&nbsp;&nbsp;{l s='My webserver holds them in his binaries path' mod='tgg_atos'}</label>
	<br />
	<br />
